#!/usr/bin/env bash
#
# Abash22 — Advanced Bash Friendly Terminal
# Author: Sagor Ahmed
#
...
# search by name or by content, open files by description, navigate with
# a real GUI-style split-screen dashboard (tmux), and drop into any raw
# bash command whenever you want. Run `help` inside it for the full
# command reference with examples.
#
abash_version="22.01-g"
...
# require_login -> optional password gate, run once by the OUTER process
# only (the tmux inner pane and the plain fallback both trust it).
# Verifies via `su <yourself> -c true`: `su` always asks for the target
...
is_scroll_down() {
    { { has_prefix_word "$1" "scroll" || has_word "$1" "page"; } && has_word "$1" "down"; } || \
    { has_word "$1" "next" && has_word "$1" "page"; }
}
is_scroll_up() {
    { { has_prefix_word "$1" "scroll" || has_word "$1" "page"; } && has_word "$1" "up"; } || \
    { has_word "$1" "previous" && has_word "$1" "page"; } || { has_word "$1" "prev" && has_word "$1" "page"; }
}
is_scroll_top()    { has_prefix_word "$1" "scroll" && { has_word "$1" "top" || has_word "$1" "beginning" || has_word "$1" "start"; }; }
is_scroll_bottom() { has_prefix_word "$1" "scroll" && { has_word "$1" "bottom" || has_word "$1" "end"; }; }
...
# ---------------------------------------------------------------------------
# 5. Visual directory listing — icons by file type. Used ONLY by the tmux
#    right-hand dashboard; the left console is a plain, real terminal, so
#    icons don't belong there. The dashboard pane sources these exact
#    function bodies via `declare -f` (see launch_tmux_gui) instead of a
#    hand-duplicated copy, so it can never drift out of sync.
# ---------------------------------------------------------------------------
...
file_mtime_str() {
    if [ "$(uname)" = "Darwin" ]; then stat -f '%Sm' -t '%m-%d %H:%M' "$1" 2>/dev/null; else stat -c '%y' "$1" 2>/dev/null | cut -c6-16; fi
}

icon_for() {
    local name="$1" isdir="$2" isexec="$3"
    if [ "$isdir" = "1" ]; then printf '📁'; return; fi
    if [ "$isexec" = "1" ]; then printf '🚀'; return; fi
    case "${name##*.}" in
        pdf) printf '📕' ;;
        jpg|jpeg|png|gif|heic|bmp|svg|webp) printf '🖼️' ;;
        mp4|mov|mkv|avi|webm) printf '🎬' ;;
        mp3|wav|m4a|flac|aac) printf '🎵' ;;
        zip|tar|gz|tgz|tbz2|dmg|rar|7z) printf '🗜️' ;;
        txt|md|rtf|log) printf '📝' ;;
        doc|docx) printf '📄' ;;
        xls|xlsx|csv) printf '📊' ;;
        ppt|pptx|key) printf '📽️' ;;
        sh|py|js|ts|rb|c|cpp|go|java|rs|swift) printf '⚙️' ;;
        *) printf '📄' ;;
    esac
}
...
human_size() {
    ...
    if [ "$bytes" -lt 1024 ]; then printf '%dB' "$bytes"
    elif [ "$bytes" -lt 1048576 ]; then printf '%dK' $((bytes / 1024))
    elif [ "$bytes" -lt 1073741824 ]; then printf '%dM' $((bytes / 1048576))
    else printf '%dG' $((bytes / 1073741824))
    fi
}

# render_visual_listing DIR HIDDEN(0/1) DETAILED(0/1) [START] [LIMIT]
# With START/LIMIT omitted, renders everything plus a "Total: N" footer.
# With them given (used by the paginated right-hand pane), renders just
# that slice and leaves the footer to the caller; the full item count is
# left in the global RVL_TOTAL for the caller to build its own footer from.
render_visual_listing() {
    local dir="$1" hidden="$2" detailed="$3" start="${4:-0}" limit="${5:-0}"
    local -a items=()
    if [ "$hidden" = "1" ]; then
        while IFS= read -r f; do items+=("$f"); done < <(ls -1A "$dir" 2>/dev/null)
    else
        while IFS= read -r f; do items+=("$f"); done < <(ls -1 "$dir" 2>/dev/null)
    fi
    RVL_TOTAL=${#items[@]}
    if [ "$RVL_TOTAL" -eq 0 ]; then
        RVL_FOLDERS=0
        RVL_FILES=0
        echo "  (empty)"
        return
    ...
    RVL_FILES=0
    for it in "${items[@]}"; do
        if [ -d "$dir/$it" ]; then RVL_FOLDERS=$((RVL_FOLDERS + 1)); else RVL_FILES=$((RVL_FILES + 1)); fi
    done

    local end
    if [ "$limit" -gt 0 ]; then
        end=$((start + limit - 1))
        [ "$end" -ge "$RVL_TOTAL" ] && end=$((RVL_TOTAL - 1))
    else
        start=0
        end=$((RVL_TOTAL - 1))
    fi

    local idx name full icon type size mdate isexec
    idx=$start
    while [ "$idx" -le "$end" ]; do
        name="${items[$idx]}"
        full="$dir/$name"
        isexec=0
        [ -f "$full" ] && [ -x "$full" ] && isexec=1
        if [ "$detailed" = "1" ]; then
            if [ -d "$full" ]; then
                icon=$(icon_for "$name" 1 0); type="folder"; size="-"
            else
                icon=$(icon_for "$name" 0 "$isexec"); type="file"
                size=$(human_size "$(file_size_bytes "$full")")
            fi
            mdate=$(file_mtime_str "$full")
            printf "  %s %-28s %-7s %6s  %s\n" "$icon" "$name" "$type" "$size" "$mdate"
        else
            if [ -d "$full" ]; then printf '  %s %s/\n' "$(icon_for "$name" 1 0)" "$name"
            else printf '  %s %s\n' "$(icon_for "$name" 0 "$isexec")" "$name"
            fi
        fi
        idx=$((idx + 1))
    done
    if [ "$limit" -eq 0 ]; then
        echo "  ---"
        echo "  Total: $RVL_TOTAL item(s)"
    fi
}

# ---------------------------------------------------------------------------
# 6. Smart file search & open — Spotlight on macOS, locate/find on Linux
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. Raw shell passthrough — never have to leave Abash for real work
# ---------------------------------------------------------------------------
...
print_help() {
    ...
    printf "%s\n" "${C_YELLOW}Split-screen View (tmux)${C_RESET}"
    printf "  %-58s %s\n" 'scroll down / scroll up' "page the right-hand file view"
    printf "  %-58s %s\n" 'scroll to top / scroll to bottom' "jump to either end"
    printf "  %-58s %s\n" '(also works with your mouse wheel over that pane)' ""
    printf "  %-58s %s\n" '(the icon file browser lives ONLY on the right —' ""
    printf "  %-58s %s\n" ' the left console stays a plain, real terminal)' ""
...
abash() {
    ...
    # --- scroll the split-screen directory view -----------------------------
    elif is_scroll_top "$input" || is_scroll_bottom "$input" || is_scroll_down "$input" || is_scroll_up "$input"; then
        if [ -z "$SCROLL_FILE" ]; then
            print_warn "Scrolling only applies in the split-screen (tmux) view."
        else
            local scmd
            if is_scroll_top "$input"; then scmd="top"
            elif is_scroll_bottom "$input"; then scmd="bottom"
            elif is_scroll_down "$input"; then scmd="down"
            else scmd="up"
            fi
            printf '%s' "$scmd" > "$SCROLL_FILE" 2>/dev/null
            print_success "Scrolling $scmd in the directory view."
        fi
    ...
    # Distinct from "back to X" (a synonym for "go to X"), handled below.
        ...
        cd .. || true
        [ -n "$STATE_FILE" ] && printf '%s' "$PWD" > "$STATE_FILE" 2>/dev/null
    ...
    # The left console is a real terminal — plain colourised `ls`, not the
    # icon file-browser. The icon view (render_visual_listing) is reserved
    # for the right-hand split-screen dashboard only.
...
# ---------------------------------------------------------------------------
# 10. Best-effort Tab completion. Path completion via readline always works
...
ABASH_KEYWORDS="help commands make create directory directories folder folders location locations name names go cd navigate jump change open launch play pwd where current show display list full hidden all delete remove erase trash undo permanently rm copy duplicate cp move rename mv touch file cat read search find grep word text tree size usage free disk space process processes running count how many compress zip extract unzip executable chmod scroll page top bottom down up next previous write append alias as ip ping download fetch clear cls history whoami date time exit quit close terminate"
...
# ---------------------------------------------------------------------------
# 11. Main REPL (shared by both the tmux inner pane and the plain fallback)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 12. GUI launcher — real split-screen via tmux when available.
#     Left pane: this same script, running the normal REPL.
#     Right pane: a live file browser (icons, sticky header, scrollable via
#     the left console's "scroll" commands) of whatever directory the left
#     pane is currently in — kept in sync via a shared state file.
#     Falls back to a plain single pane if tmux isn't installed, already
#     inside tmux, or not attached to a real terminal.
# ---------------------------------------------------------------------------

launch_tmux_gui() {
    local session="abash${abash_version}_$$"
    local state_file="/tmp/.abash${abash_version}_state_$$"
    local scroll_file="/tmp/.abash${abash_version}_scroll_$$"
    local icon_helpers="/tmp/.abash${abash_version}_icons_$$.sh"
    local right_script="/tmp/.abash${abash_version}_right_$$.sh"

    printf '%s' "$PWD" > "$state_file"
    : > "$scroll_file"

    # Dump the ACTUAL running function bodies (not a hand-copied duplicate)
    # so the right pane can never drift out of sync with the left
    # console's icon/size/date logic.
    declare -f icon_for human_size file_size_bytes file_mtime_str render_visual_listing > "$icon_helpers"
...
if [ -t 1 ] && command -v tput >/dev/null 2>&1 && [ "\$(tput colors 2>/dev/null || echo 0)" -ge 8 ]; then
    RC=\$(tput setaf 6); RB=\$(tput bold); RD=\$(tput dim); RR=\$(tput sgr0)
else
    RC=""; RB=""; RD=""; RR=""
fi
...
while true; do
    ...
    cmd=\$(cat "\$SCROLL_FILE" 2>/dev/null)

    rows=\$(tput lines 2>/dev/null || echo 40)
    header_h=6
    footer_h=2
    list_h=\$((rows - header_h - footer_h))
    [ "\$list_h" -lt 1 ] && list_h=1

    # Page step scales with the visible area instead of a fixed number.
    page_step=\$((list_h / 2))
    [ "\$page_step" -lt 3 ] && page_step=3

    changed=0
    [ "\$dir" != "\$last_dir" ] && changed=1
    if [ -n "\$cmd" ]; then
        changed=1
        case "\$cmd" in
            down) offset=\$((offset + page_step)) ;;
            up) offset=\$((offset - page_step)); [ "\$offset" -lt 0 ] && offset=0 ;;
            top) offset=0 ;;
            bottom) offset=999999 ;;
        esac
        : > "\$SCROLL_FILE"
    fi
    now=\$(date +%s)
    [ \$((now - last_redraw)) -ge 30 ] && changed=1

    # Only redo the EXPENSIVE stuff (re-listing the directory, du -sh,
    # df -h) when something changed (directory, a scroll command) or
    # every 30s to keep them honest — a full clear+redraw every single
    # second would just flicker for no reason. The clock still ticks
    # every second below regardless, via `tput cup` to just that one
    # line, so it still reads as a live clock rather than a frozen
    # timestamp.
    if [ "\$changed" -eq 1 ]; then
        last_dir="\$dir"
        last_redraw=\$now

        # Folder/file breakdown and total size are computed directly here
        # rather than reused from render_visual_listing's own counting
        # (RVL_TOTAL etc.), because that function is called below via
        # command substitution — a function's global-variable side
        # effects don't survive being called inside \$(...), since that
        # runs in a subshell.
        total_count=\$(ls -1A "\$dir" 2>/dev/null | wc -l | tr -d ' ')
        folder_count=\$(find "\$dir" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
        file_count=\$((total_count - folder_count))
        size=\$(du -sh "\$dir" 2>/dev/null | awk '{print \$1}')
        free=\$(df -h "\$dir" 2>/dev/null | awk 'NR==2{print \$4}')

        clear
        echo "\${RC}\${RB}╔══════════════════════════════════════════╗\${RR}"
        echo "\${RC}\${RB}║  🗂  ABASH${abash_version} — Live Directory View       ║\${RR}"
        echo "\${RC}\${RB}╚══════════════════════════════════════════╝\${RR}"
        echo " \${RB}Time\${RR} : \$(date '+%Y-%m-%d %H:%M:%S')"
        echo " \${RB}Dir \${RR} : \$dir"
        echo " \${RB}Size\${RR} : \${size:-?}   \${RB}Free\${RR}: \${free:-?}   \${RB}Items\${RR}: \$folder_count folder(s), \$file_count file(s)"
        echo "\${RD}──────────────────────────────────────────────\${RR}"

        render_visual_listing "\$dir" 1 1 "\$offset" "\$list_h"
        total="\$RVL_TOTAL"
        max_off=\$((total - list_h))
        [ "\$max_off" -lt 0 ] && max_off=0
        [ "\$offset" -gt "\$max_off" ] && offset=\$max_off

        echo "\${RD}──────────────────────────────────────────────\${RR}"
        if [ "\$total" -gt "\$list_h" ]; then
            end=\$((offset + list_h))
            [ "\$end" -gt "\$total" ] && end=\$total
            echo " Showing \$((offset + 1))-\$end of \$total — scroll the mouse wheel here, or say 'scroll down'/'scroll up'"
        else
            echo " (\$total item(s) shown)"
        fi
    else
        # Cheap per-second update — move the cursor to just the Time line
        # and overwrite it, leaving the rest of the screen (the listing,
        # free space, etc.) untouched until the next real redraw. This is
        # what makes the clock feel alive without paying for a full
        # re-list + du/df every second.
        tput cup 3 0
        printf " %sTime%s : %s" "\$RB" "\$RR" "\$(date '+%Y-%m-%d %H:%M:%S')"
        tput el
    fi
    sleep 1
done
EOF
    ...
    tmux new-session -d -s "$session" \
        ...
        "bash '$0' --inner '$state_file' '$scroll_file'; tmux kill-session -t '$session' 2>/dev/null"
    tmux split-window -h -t "${session}:0" -p 38 "bash '$right_script'"
    tmux select-pane -t "${session}:0.0"
    tmux set-option -t "$session" pane-border-status top 2>/dev/null
    tmux select-pane -t "${session}:0.0" -T "Abash${abash_version} — Command Console" 2>/dev/null
    tmux select-pane -t "${session}:0.1" -T "Live Directory View" 2>/dev/null
    # Without these, the LEFT pane doesn't feel like a real terminal —
    # the mouse wheel doesn't scroll it, and the default 2000-line
    # scrollback runs out fast in a busy session.
    tmux set-option -t "$session" mouse on 2>/dev/null
    tmux set-option -t "$session" history-limit 10000 2>/dev/null

    # By default, `mouse on` makes the wheel enter tmux's own copy-mode
    # scrollback on ANY pane — over the right-hand dashboard that just
    # replays old redraws, which looks broken. Rebind the wheel, but only
    # while the active pane is this session's dashboard (index 1), to
    # page it directly through the scroll file instead.
    #
    # IMPORTANT: the fallback for every OTHER pane must be tmux's own
    # stock WheelUpPane behaviour, not a plain `send-keys -M` — tmux's
    # real default is conditional: forward the raw event only if the
    # program is mouse-aware/in an alternate screen/already in copy-mode,
    # and otherwise enter copy-mode (`copy-mode -e`); that "otherwise" is
    # what makes a plain shell's pane scroll at all (verify with `tmux
    # list-keys -T root` on an unmodified tmux). Nesting that exact stock
    # condition as the fallback keeps the left pane — and any pane in any
    # other window — scrolling exactly like default tmux.
    tmux bind-key -n WheelUpPane if-shell -F "#{&&:#{==:#{session_name},$session},#{==:#{pane_index},1}}" \
        "run-shell 'echo up > \"$scroll_file\"'" \
        "if-shell -F \"#{||:#{alternate_on},#{pane_in_mode},#{mouse_any_flag}}\" { send-keys -M } { copy-mode -e }" 2>/dev/null
    tmux bind-key -n WheelDownPane if-shell -F "#{&&:#{==:#{session_name},$session},#{==:#{pane_index},1}}" \
        "run-shell 'echo down > \"$scroll_file\"'" "send-keys -M" 2>/dev/null

    # Keep re-attaching for as long as the session exists — a plain
    # `tmux attach` can return the moment the *client* detaches (Ctrl-B D,
    # or a flaky connection) even though the session and both panes are
    # still very much alive; cleaning up the shared files at that point
    # would pull the rug out from under a session the user hasn't
    # actually asked to end.
    while tmux has-session -t "$session" 2>/dev/null; do
        tmux attach -t "$session" 2>/dev/null
    done

    rm -f "$state_file" "$scroll_file" "$icon_helpers" "$right_script" 2>/dev/null
}
...
if [ "$1" = "--inner" ]; then
    # Already gated by the outer process before it ever launched tmux —
    # don't ask again for the pane tmux spawns.
    STATE_FILE="$2"
    SCROLL_FILE="$3"
    run_repl
elif command -v tmux >/dev/null 2>&1 && [ -z "$TMUX" ] && [ -t 0 ] && [ -t 1 ]; then
    require_login || exit 1
    launch_tmux_gui
else
    if ! command -v tmux >/dev/null 2>&1; then
        print_info "Tip: '$(pkg_install_hint tmux)' enables the live split-screen dashboard."
    fi
    require_login || exit 1
    STATE_FILE=""
    SCROLL_FILE=""
